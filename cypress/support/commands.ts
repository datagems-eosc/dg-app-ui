				...
				const rewritten = cookieArray
					...
					.map((headerContent: string) =>
						...
						headerContent
							...
							.replace(/(?<!samesite=)(;?\s*)$/i, (m) => (headerContent.toLowerCase().includes('samesite=') ? m : `${m}; SameSite=None; Secure`))
					);
...
declare global {
	namespace Cypress {
		interface Chainable {
			 ...
			 */
			login(opts?: { keycloakOrigin?: string; visitPath?: string }): Chainable<void>;
		}
	}
}
...
Cypress.Commands.add('login', (opts = {}) => {
	...
	const keycloakOrigin = opts.keycloakOrigin || Cypress.env('KEYCLOAK_ORIGIN') || 'https://datagems-dev.scayle.es';
	...
	cy.fixture('credentials')
		.then((creds: any) => {
			const username = creds?.username || Cypress.env('USERNAME') || Cypress.env('CYPRESS_USERNAME');
			const password = creds?.password || Cypress.env('PASSWORD') || Cypress.env('CYPRESS_PASSWORD');
			...
			cy.visit(visitPath.startsWith('http') ? visitPath : `${baseUrl}${visitPath}`);
			cy.contains(/Sign in to DataGEMS/i, { timeout: 10000 }).should('be.visible');

			// Stub the external authChecker script (matches existing test pattern)
			cy.intercept(
				'GET',
				`${keycloakOrigin}/oauth/resources/**/authChecker.js`,
				{
					statusCode: 200,
			...
			cy.origin(keycloakOrigin, { args: { username, password } }, (args) => {
				...
				cy.contains(/Or sign in with/i, { timeout: 20000 }).should('be.visible').click();

				cy.get('input[placeholder="Username or email"], input[name="username"], input#username', { timeout: 20000 })
					.should('be.visible')
					.type(u);

				cy.get('input[placeholder="Password"], input[name="password"], input#password', { timeout: 20000 })
					.should('be.visible')
					.type(p, { log: false });

				cy.get('input#kc-login, input[name="login"], input[value="Sign In"], button[type="submit"]', { timeout: 10000 })
					.first()
					.click();
			});
