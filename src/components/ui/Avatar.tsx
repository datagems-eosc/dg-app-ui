...
interface AvatarProps {
  ...
  const sizeClasses = {
    ...
    md: 'w-12 h-12 text-body-16-regular',
    lg: 'w-16 h-16 text-body-16-regular',
    xl: 'w-24 h-24 text-H6-18-semibold'
  };
...
function getAvatarColor(name?: string, email?: string): string {
  ...
  const str = (name && name.trim()) || (email && email.trim()) || 'Marry Johnson';
...
export function Avatar({ src, name, email, size = 'md', className = '' }: AvatarProps) {
  ...
  return (
    <div 
      className={`${sizeClass} rounded-full flex items-center justify-center text-white text-body-16-semibold ${bgColor} ${className}`}
    >
      {initials}
    </div>
  );
        ...
        {!disabled && (
          <div className="absolute inset-0 rounded-full transition-all duration-200 flex items-center justify-center opacity-0 group-hover:opacity-100 group-hover:bg-black group-hover:bg-opacity-50">
            <div className="text-center">
              ...
              <div className="text-descriptions-12-semibold text-white">Upload</div>
            </div>
          </div>
        )}
      </div>
